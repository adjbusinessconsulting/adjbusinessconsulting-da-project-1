...
| Step | Result |
|------|--------|
| Check missing values | Missing values found in CustomerID — rows with null CustomerID were dropped |
| Check duplicate rows | No exact duplicate rows found |
| Convert data types | InvoiceDate converted to datetime format |
| Remove invalid values | Rows with negative or zero Quantity and UnitPrice removed |
| Remove cancelled transactions | Cancelled invoices (starting with 'C') identified and excluded from RFM calculation |
| Final dataset shape | Cleaned dataset ready for RFM feature engineering |

## What We Did
Performed in-depth EDA on the UCI Online Retail dataset, then calculated RFM scores per customer. Each customer was given a score of 1–5 for Recency, Frequency, and Monetary value — then grouped into 4 strategic segments and visualized in an interactive Power BI dashboard.
